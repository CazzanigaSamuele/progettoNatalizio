...
if (isset($_GET['numDomande'])) { // Verifica se il parametro 'numDomande' è stato passato nella richiesta GET
    ...
    $nomeQuiz = $_GET["nomeQuiz"]; // Estrae il nome del quiz
    ...
    if ($q->controllaSeNomeQuizPresente($nomeQuiz)) {
        header('Location: paginaCreazioneQuizUtente.php?numDomande=' . $numDomande . '&messaggio=nome quiz già presente!!!');
    ...
    for ($i = 1; $i <= $numDomande; $i++) {
        ...
        if ($flag == false) {
            ...
            header('Location: paginaCreazioneQuizUtente.php?numDomande=' . $numDomande . '&messaggio=Opzione corretta deve essere presente nelle opzioni!!!');
