...
class CatalogoQuiz 
{
    ...
    public function eliminaQuizTramiteNome($nomeQuiz) {
        ...
        // Elimina i punteggi relativi al quiz dal file CSV "punteggi"
        $filePunteggi = 'csv/punteggi.csv';
        $nuoviPunteggi = [];

        $contenuto = file_get_contents($filePunteggi);
        $righe = explode("\n", $contenuto);

        foreach ($righe as $riga) {
            $colonne = explode(";", $riga);

            if ($colonne[1] != $nomeQuiz) {
                $nuoviPunteggi[] = $riga;
            }
        }

        // Salva i punteggi aggiornati nel file CSV
        file_put_contents($filePunteggi, implode("\n", $nuoviPunteggi));
